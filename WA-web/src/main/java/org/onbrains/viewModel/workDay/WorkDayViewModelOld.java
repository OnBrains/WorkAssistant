...
public class WorkDayViewModelOld {
	...
	/**
	 * В Primefaces если изменять только время, без года, то проставляется 1970 год. Если ортредактированно время
	 * прихода/ухода, то надо проставить год обратно.
	 */
	private Calendar updateYear(WorkDay workDay, Calendar calendar) {
		Calendar currentDay = Calendar.getInstance();
		currentDay.setTime(workDay.getDay().getDay());
		calendar.set(currentDay.get(currentDay.YEAR), currentDay.get(currentDay.MONTH),
				currentDay.get(currentDay.DATE));
		return calendar;
	}
