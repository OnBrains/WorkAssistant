...
public class CurrentDayFrameModelOld implements Serializable {
	...
	private Long getPossibleOutTimeInMSecond() {
		if (getCurrentDay() != null) {
			...
			Long possibleOutTimeInMSecond = commingTimeInMSeconds
					+ currentDay.getDay().getType().getWorkTimeInMSecond();
		...
		if (isWorking()) {
			if (isWorked()) {
				...
				return DateFormatService
						.mSecToHHMM(currentTime.getTimeInMillis() - currentDay.getComingTime().getTimeInMillis());
			}
		} else {
			return "__:__";
		}
	...
	public String getLegendValue() {
		return getCurrentDay() != null
				? DateFormatService.toDDEE(currentDay.getDay().getDay()) + " - " + currentDay.getState().getDesc()
				: "Не найдено";
	}
